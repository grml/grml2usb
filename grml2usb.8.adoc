...
Finally create a filesystem and execute grml2usb as needed:
  ...
  # mkfs.vfat /dev/loop1
  # grml2usb --bootloader-only /grml/isos/grml-full-2025.08-amd64.iso /dev/loop1
...
~~~~~~~~~~~~~~~~~~~
  ...
  # grml2usb grml-full-2025.08-amd64.iso /dev/sdX1
...
system available) running:
  ...
  % dd if=grml-full-2025.08-amd64.iso of=/dev/sdX
...
--------------
  ...
  # grml2usb /home/grml/grml-full-2025.08-amd64.iso /dev/sdX1

Install specified ISO on device /dev/sdX1.

  # grml2usb /home/grml/grml-full-2025.08-amd64.iso /home/grml/grml-full-2025.08-arm64.iso  /dev/sdX1
...
Install currently running Grml live system on device /dev/sdX1.
  ...
  # grml2usb /run/live/medium /home/grml/grml-full-2025.08-amd64.iso /dev/sdX1

Install currently running Grml live system and the specified
ISO on device /dev/sdX1 for multibooting.

  # grml2usb --format /home/grml/grml-full-2025.08-amd64.iso /dev/sdX1

Install specified ISO on device /dev/sdX1 and format partition /dev/sdX1 with
FAT filesystem.

  # parted /dev/sdX --script mklabel gpt mkpart P1 fat32 0 100% set 1 boot on set 1 esp on
  # grml2usb --format /home/grml/grml-full-2025.08-amd64.iso /dev/sdX1

Manually partition the `/dev/sdX` drive before installing the ISO,
which grml2usb currently expects to be done separately, but might
eventually do on its own. Note that this will destroy data on
`/dev/sdX`!

  # grml2usb --grub --grub-mbr /home/grml/grml-full-2025.08-amd64.iso /dev/sdX1
...
/dev/sdX.
  ...
  # grml2usb --bootoptions="lang=de ssh=mysecret" grml-full-2025.08-amd64.iso /dev/sdX1
  # grml2usb --bootoptions="lang=de" --bootoptions="ssh=mysecret" grml-full-2025.08-amd64.iso /dev/sdX1

Install specified ISO on device /dev/sdX1 and use "lang=de ssh=mysecret" as
default bootoptions.


  # grml2usb --remove-bootoption="vga=791" --remove-bootoption="nomce" grml-full-2025.08-amd64.iso /dev/sdX1

Install specified ISO on device /dev/sdX1 remove vga=791 and nomce from existing bootoptions.

  # grml2usb --bootoptions="persistent-path=%flavour_name" grml-full-2025.08-amd64.iso grml-full-2025.08-arm64.iso /dev/sdX1
